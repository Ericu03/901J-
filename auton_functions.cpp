...
void base_PID(double targetdistance, int velocity){
  leftenc.reset();
  rightenc.reset();
  const double degreeGoal = (targetdistance/CIRCUMFERENCE) * TICKS_PER_ROTATION; // Ticks of rotation needed
  bool goalmet = false;
  double goal = 0.0;
  double target = 0.0;
  ...
  error = degreeGoal; // Rotation ticks away from goal
...
//}
  ...
  //derivative = error - prevError; // Error improvment
//  prevError = error;
targetvelocity = kP*error;  //+ kI*integral  + kD*derivative ;

  if(targetvelocity > maxvelocity){ // Cap the speed
